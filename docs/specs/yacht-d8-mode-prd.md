...
| 상단 보너스 임계 | 63점 | **112점** (windfall 발생 시에도 나머지 평균 이상 요구) |
...
- **임계값**: 상단 8개 합계 **≥ 112점** (실 플레이 기반 상향).
- **보너스 점수**: **+35점** (D6와 동일).
- **판정 시점**: 상단 8개 모두 기록된 시점에 자동 판정 + `SCORE_RECORDED.payload.bonusEarned=true` 알림.
- 8개 미만 기록 시 미확정 (현재 합계만 미리보기).
- **균형 이론**: 카테고리당 평균 3.11개 기준(112÷36). SEVENS·EIGHTS에서 4개 이상 windfall(~19% 확률)이 발생해도 나머지 7개 카테고리에서 평균 이상 성능이 필요한 수준. 비례 임계(108)는 킵 메커니즘·카테고리 선택 유연성·고면값 폭발력을 과소평가해 체감 난이도가 D6보다 현저히 낮아 상향.
...
| OQ-2 | 상단 보너스 임계 적정성 | 비례값 108은 킵 메커니즘·카테고리 선택 유연성·SEVENS·EIGHTS 고면값 폭발력 과소평가로 실 플레이에서 D6보다 훨씬 쉬움. windfall(7·8 넷 이상, ~19%) 발생 시 나머지 평균 이상 요구 수준인 112가 적정. | **112점으로 확정.** (2026-05-12) |
