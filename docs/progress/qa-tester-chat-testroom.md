...
## 2026-04-23 (재검증 완료 — 최종 PASS)

### 상태
정적 검증 완료. 최종 판정: **PASS**

### 완료된 산출물
- `docs/review/chat-testroom-test-plan.md`
  - TC 약 40개 (보안 13, 방 생성/목록 8, 실시간 채팅 8, 재연결 3, Redis 3, ADMIN 4, 엣지케이스 7, Graceful Degradation 4, HomePage 3, 성능 2)
  - EC-1 ~ EC-18 전수 TC 매핑 (섹션 10)

### 재검증 결과 요약

| 등급 | 1차 발견 | 재검증 결과 |
|------|---------|------------|
| CRITICAL | 1 | 수정 확인 — StompChannelInterceptor.java `import java.util.Map` 추가 완료 |
| HIGH | 4 | 수정 확인 — 메시지 차단 로직, roomId 패턴 검증, 다중 방 퇴장, 핸드셰이크 우회 방지 모두 반영 |
| MEDIUM | 2 | 수정 확인 — 퇴장 메시지 TTL 갱신 부작용 및 FriendRoute 화이트리스트 방식 전환 완료 |
| LOW | 2 | 잔존 — 런타임 TC 실행 시 추가 검증 필요 (우선순위 낮음, 배포 차단 없음) |

### 최종 판정
**PASS** — CRITICAL 1건, HIGH 4건, MEDIUM 2건 수정 확인 완료. LOW 2건은 런타임 환경에서 추가 검증 예정이나 배포 차단 수준 아님.

### 남은 작업
- 런타임 TC (TC-SEC-01 ~ TC-CONN-03, 주요 보안/채팅 흐름) 는 backend + frontend 로컬 실행 환경 구성 후 진행 필요
- LOW 버그 2건 런타임 재현 시도:
  - [DbgChatRoomPage.tsx] 재연결 실패 후 토스트 미표출 가능 여부
  - [ChatController.java] SockJS 폴백 환경에서 Principal 타입 불일치 여부
- 회귀 smoke test: 기존 RSP, Blockfall, Blockfall Insane 동작 확인 (로컬 환경 구성 후)

### 커밋 정보
- WIP 브랜치 커밋 완료: `5d0f99b`
- 산출물: `docs/review/chat-testroom-test-plan.md`, `docs/review/chat-testroom-bugs.md`
