...
- 최초 작성일: 2026-04-24
- 상태: **세션 완료 — QA PASS, 커밋 367c623 (2026-04-24)**
...
| `frontend/src/games/online-rps/components/GameScreen.tsx` | 게임 화면 컴포넌트 (10초 타이머, 카드 선택, 키보드 단축키 R/P/S) |
...
### 키보드 단축키 (GameScreen.tsx)

- `R` 키: 바위(ROCK) 선택
- `P` 키: 보(PAPER) 선택
- `S` 키: 가위(SCISSORS) 선택
- playing phase에서만 활성화, 이미 선택한 경우 무시

---

## BUG-3 확인 결과

- `/games/rcp/` 경로: App.tsx 라우트 정상 등록 확인
- 수정 불필요 판단 (2026-04-24)
...
| eslint 에러 | 0 |
| QA 결과 | PASS |
| 다음 담당 | developer-backend (WebSocket 핸들러, POST /api/rps/match 구현) |
...
2. 통합 테스트 중 발견되는 버그 수정
3. 필요 시 키보드 단축키 힌트 UI 추가 (현재는 단축키만 구현, 화면 표시 없음)
