...
public class StompChannelInterceptor implements ChannelInterceptor {
    ...
    // Battle 경로 — StompChannelInterceptor 검사 제외 (별도 인터셉터가 처리)
    private static final Pattern BATTLE_TOPIC_PATTERN = Pattern.compile(
            "^/topic/(blockfall-battle|minesweeper-battle|apple-battle)/.*$");
    private static final Pattern BATTLE_APP_PATTERN = Pattern.compile(
            "^/app/(blockfall-battle|minesweeper-battle|apple-battle)/.*$");
