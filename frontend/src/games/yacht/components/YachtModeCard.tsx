...
const MODE_INFO: Record<DiceType, {
  ...
  D8: {
    ...
    scoreCount: 14,
    roundMultiplier: 14,
    ariaLabel: 'D8 정팔면체 모드 선택, 14 족보, 보너스 108점',
  },
};
...
export default function YachtModeCard({
  ...
  return (
    <div
      className={`${styles.modeCard} ${diceType === 'D8' ? styles.modeCardD8 : styles.modeCardD6}`}
      role="button"
      tabIndex={0}
    ...
    >
      ...
      {/* 모드명 */}
      <p className={`${styles.modeCardTitle} ${diceType === 'D8' ? styles.modeCardTitleD8 : styles.modeCardTitleD6}`}>
        {info.title}
      </p>
      ...
      <button
        type="button"
        className={`${styles.modeCardCta} ${diceType === 'D8' ? styles.modeCardCtaD8 : ''}`}
